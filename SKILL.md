...
- **权限系统（2026-08-06 实施，LuckPerms 4 组）**：default(新手生存)→member(进阶飞行)→builder(创造+WE)→admin(全权限)；坑：RCON 不回显 LP 命令输出（用 bot 玩家身份验证）、default 勿显式设 false 覆盖子组 true、Essentials 权限默认拒绝。**2026-08-29 实测补：gamemode 权限真值（essentials.gamemode 子权限独立）+ admin 补 essentials.gamemode.spectator（PR#229）+ builder/admin 降级后 gamemode 矫正方案立项**。**2026-08-30 实测补：builder 组无 `worldedit.limit` 权限（//limit 显示 Unknown/被隐藏）→ 单次编辑 100 万方块为硬上限，玩家无法调大（配合 WE 限额方案 A）**。详见 references/permission-system.md
