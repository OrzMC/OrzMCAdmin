...
> 🔄 **本地测试服拓扑 = MCSM 本机栈（2026-09-03 全量迁移完成）**：原裸跑 `~/minecraft-server`/`~/folia-test`（start.sh/screen 启动）已删除，测试服改由本机 MCSM 面板 **mcs.{SERVER_NAME}.cn** 管理——双 Docker 实例 **`papermc-test`**（uuid `716c2fb712154c36ba5ab0f1480d3f87`，Paper 26.2）与 **`folia-test`**（uuid `8A932DD47F4D42AAAD6A6A9A5FAD2A91`，Folia 26.2），共享 world `/Users/Shared/orzmc/worlds/test`（严禁同跑），实例数据在宿主 `/Users/Shared/orzmc/mcsmanager/daemon/data/InstanceData/<uuid>/`（含 server.properties/plugins/logs 完整服务端，**宿主侧直接可见可操作**）。**本 SKILL 中所有「本地测试服/本地双服/本地已改+重启」如无特别说明均指这两个实例**（jar/插件部署 = 直接 cp 实例目录 + 面板重启实例；启停 = 面板操作）。五连坑/克隆实例法/详细路径见 `references/testing.md`「MCSM 本地栈 Docker 实例」节。
