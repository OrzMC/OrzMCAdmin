#!/usr/bin/env node
/**
 * HermesBot - Mineflayer 机器人玩家（2026-08-04 实测可用，含粒子 patch + 自动重连）
...
const HOST = process.env.BOT_HOST || process.argv[3] || 'localhost';
...
function createBot() {
  bot = mineflayer.createBot({
    ...
    username: USER,
    auth: 'offline',
    // ⚠️ 服务器是 MC 26.2，但 minecraft-protocol 最高支持 1.21.11：
    // 用 1.21.11 连接，服务器 ViaVersion 自动转换（用 '26.2' 会报 unsupported protocol version）
    version: '1.21.11',
    hideErrors: false,
  });
